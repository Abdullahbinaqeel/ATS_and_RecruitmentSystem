...
## STEP 9 — `backend/models/PracticeInterview.js`
**Created ninth because:** It defines the database schema for AI mock interview sessions, linking a candidate and their application to a specific job and recording the message history.

**What it defines:**
- `candidate` — ref to User (must be candidate)
- `job` — ref to Job
- `application` — ref to Application
- `status` — 'in_progress' | 'completed'
- `messages` — array of sub-documents representing the chat history:
  - `role` — 'user' | 'model' (matching Gemini's role requirements)
  - `parts` — array of `{ text: String }` (matching Gemini's content parts format)
- `feedback` — review generated by Gemini upon completing the practice
- `score` — mock score out of 100 generated by Gemini

---

## STEP 10 — `backend/middleware/auth.js`
**Created tenth because:** All protected routes need this middleware.
...
## STEP 11 — `backend/middleware/upload.js`
**Created eleventh because:** Auth routes and application routes need file upload
...
## STEP 12 — `backend/utils/email.js`
**Created twelfth because:** The HR routes need to send emails, but this
...
## STEP 13 — `backend/utils/gemini.js`
**Created thirteenth because:** Before creating practice interview API routes, we need the utility function that handles communication with the Google Gemini API.

**What it exports:**
- `generateNextPracticeMessage(systemPrompt, chatHistory)` — calls the Gemini 2.5 Flash API to get the interviewer's next response, with offline/empty history fallbacks.
- `evaluatePracticeInterview(systemPrompt, chatHistory)` — calls the Gemini 2.5 Flash API with a structured prompt requesting a JSON response containing feedback and a numerical score.

---

## STEP 14 — `backend/routes/auth.js`
**Created fourteenth because:** Authentication must exist before any protected routes.
...
## STEP 15 — `backend/routes/branches.js`
**Created fifteenth because:** Jobs reference branches, so branch CRUD must exist
...
## STEP 16 — `backend/routes/jobs.js`
**Created sixteenth:** Job listings are the core product feature.
...
## STEP 17 — `backend/routes/applications.js`
**Created seventeenth:** Applications link candidates to jobs — depends on both existing.
...
## STEP 18 — `backend/routes/interviews.js`
**Created eighteenth:** Interviews depend on applications existing.
...
## STEP 19 — `backend/routes/practiceInterviews.js`
**Created nineteenth because:** Handles all HTTP requests for candidate practice interview sessions, depending on the `PracticeInterview` model and `gemini` utilities.

**Endpoints:**
| Method | Path | Auth | Description |
|---|---|---|---|
| GET | `/api/practice-interviews/application/:applicationId` | Candidate | Retrieve current practice session (or application detail) |
| POST | `/api/practice-interviews/start` | Candidate | Initialize a new session and generate the first greeting/question |
| POST | `/api/practice-interviews/:interviewId/message` | Candidate | Submit user response and retrieve the next AI question |
| POST | `/api/practice-interviews/:interviewId/complete` | Candidate | Finalize the session, trigger Gemini evaluation, and save feedback |
| DELETE | `/api/practice-interviews/:interviewId` | Candidate | Reset/delete an active practice session to restart |

---

## STEP 20 — `backend/routes/hr.js`
**Created twentieth:** HR dashboard aggregates data from all other collections —
...
- Defines all 13 routes using React Router's `<Routes>` and `<Route>`
...
## STEP 21 — `frontend/src/pages/candidate/PracticeInterview.jsx`
**Created 21st:** Interactive mock interview interface for candidates.
- Shows target role, guidelines, and rules before starting.
- Simulates a chat window showing the AI interviewer's messages.
- Enables candidate replies with input boxes and loaders during API waits.
- Handles completion: shows live feedback and score once complete.
- Uses axios to start, send messages, complete, and restart the practice session.

---

## STEP 22 — `frontend/src/pages/hr/Dashboard.jsx`
**Created 22nd:** HR overview screen.
...
## STEP 23 — `frontend/src/pages/hr/ManageJobs.jsx`
**Created 23rd:** Full CRUD for job listings.
...
## STEP 24 — `frontend/src/pages/hr/JobApplicants.jsx`
**Created 24th:** Most complex HR page — lists all applicants for one job.
...
## STEP 25 — `frontend/src/pages/hr/ManageBranches.jsx`
**Created 25th (last) because:** Branches are the simplest CRUD — created last since
